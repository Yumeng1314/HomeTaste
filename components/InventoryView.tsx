...
const InventoryView: React.FC<InventoryViewProps> = ({ inventory, onAdd, onUpdate, onDelete, onAISuggest }) => {
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-8">
          ...
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
            ...
            <div className="relative flex-1 w-full max-w-md">
              ...
              <input 
                type="text" 
                placeholder="搜索食材名称..." 
                className="w-full bg-white border border-gray-100 pl-12 pr-4 py-3 rounded-2xl font-black text-sm outline-none focus:ring-4 focus:ring-emerald-500/5 focus:border-emerald-500/20 transition-all shadow-sm"
                value={searchTerm}
        ...
        <div className="flex flex-wrap items-center gap-3">
          {onAISuggest && (
            <button
              onClick={onAISuggest}
              className="px-6 h-14 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-700 text-white flex items-center gap-3 shadow-lg hover:shadow-emerald-500/20 active:scale-95 transition-all group"
            >
              <span className="text-xl group-hover:rotate-12 transition-transform">✨</span>
              <span className="text-[10px] font-black uppercase tracking-widest">AI 灵感推荐</span>
            </button>
          )}
          <button
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <input
                  type="text"
                  placeholder="食材名称"
                  className="w-full border-b-2 border-gray-100 py-4 text-2xl font-black focus:border-emerald-500 outline-none"
                  value={newItem.name}
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input type="number" className="w-full bg-gray-50 rounded-2xl p-4 font-black outline-none border border-transparent focus:border-emerald-500/20" value={newItem.amount} onChange={e => setNewItem({ ...newItem, amount: Number(e.target.value) })} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">单位</label>
                    <input type="text" placeholder="如：个, 克, 瓶" className="w-full bg-gray-50 rounded-2xl p-4 font-black outline-none border border-transparent focus:border-emerald-500/20" value={newItem.unit} onChange={e => setNewItem({ ...newItem, unit: e.target.value })} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {categories.map(cat => (
                    ...
                    >
                      <span className="text-xl mb-1">{getCategoryEmoji(cat)}</span>
              ...
              <button
                onClick={() => { if(!newItem.name) return; onAdd(newItem); setShowAdd(false); }}
                className="w-full py-5 bg-emerald-600 text-white rounded-[2rem] font-black text-sm mt-4 shadow-xl hover:bg-emerald-700 transition-all active:scale-95"
              >
                加入冰箱清单
              </button>
            </div>
      ...
      {filteredInventory.length === 0 ? (
        <div className="py-40 text-center space-y-4">
          ...
          <p className="font-black text-gray-400 text-sm uppercase tracking-widest">
            {searchTerm ? `没有找到 "${searchTerm}"` : '当前区域暂无食材'}
          </p>
        </div>
      ) : (
            ...
            <div key={item.id} className="bg-white p-6 rounded-[2.5rem] border border-gray-100 flex items-center justify-between group hover:shadow-xl transition-all">
              <div className="flex items-center gap-5">
                ...
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <span className={`text-[9px] font-black px-2 py-0.5 border rounded-lg uppercase tracking-tight ${getZoneColor(item.storageZone)}`}>{item.storageZone}</span>
                    <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest">{item.category}</span>
                  </div>
                </div>
              </div>
              <div className="flex flex-col items-end gap-2">
                <div className="flex items-center bg-gray-50 rounded-xl overflow-hidden border border-gray-100">
                  ...
                  <div className="px-3 font-black text-sm">{item.amount}{item.unit}</div>
